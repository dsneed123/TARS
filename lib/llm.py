...
class LLM:
    ...
    def agent(self, role: str, prompt: str, system: str, cwd: str,
        ...
        for _turn in range(max_turns):
            ...
            # Ollama ≥0.32 parses qwen3-coder tool calls natively (verified
            # 2026-07-14); no calls means the model considers itself done.
            tool_calls = msg.get("tool_calls") or []
            if not tool_calls:
                if not changed and not nudged:
                    # A "done" with zero tool use and zero changes is almost
                    # always a lazy or malformed first reply, not a finished
                    # task — push back exactly once.
                    nudged = True
                    messages.append({"role": "user", "content": (
                        "You have not used any tools or changed any files. "
                        "Do the task now using the tools. Call finish only "
                        "if the task truly requires no changes, and say why.")})
                    continue
